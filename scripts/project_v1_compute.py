...
def main():
    ...
    scenarios=[]
    for contexts in (2,1):
        encode=100*contexts*sum(m["mean"]["encode_seconds"] for m in methods.values())
        decode=100*contexts*sum(m["mean"]["decode_seconds"] for m in methods.values())
        cold=100*contexts*sum(m["mean"]["cold_load_seconds"] for m in methods.values())
        other=100*contexts*sum(m["mean"]["other_process_setup_teardown_seconds"] for m in methods.values())
        for sharing in ("one_control_per_method","one_control_trace_per_payload_context"):
            control=100*contexts*(sum(matching.values()) if sharing=="one_control_per_method" else
            ...
            scenarios.append({"contexts":contexts,"stego_units":600*contexts,"control_policy":sharing,
                "control_generation_jobs":600*contexts if sharing=="one_control_per_method" else 200*contexts,
    ...
    result={"scope":"projection only; no V2 execution or extra GPU authority","method_timings":methods,
        ...
        "caveats":["Only two development payload groups per direction and one context were measured; no confidence intervals or unseen-context throughput claim",
          ...
          "Only the planned context reduction is shown; no direction, method, payload group or budget is removed",
    ...
    json_write(review/"compute_projection.json",result)
