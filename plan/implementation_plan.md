...
**Authorized V1 qualification continuation (after accepted revision `b5f2d1e`):** complete the existing frozen development allocation without V2 or held-out carrier generation. The user adds 43,200 seconds **once**, making the absolute V1 cap 50,400 seconds; V0 stays at 7,200 seconds and the whole-project ceiling stays at 40 GPU-hours. The allowance is authorization, not an additional forecast expense. Existing charges and original paired ciphertexts/keys remain intact. The already budgeted 20 lossless PNG checks are performed during qualification on fixed-rank development carriers selected as T1–T10 under both rows; they are file-preservation replays, not new stego observations, and must not be charged again as a future main-study requirement. Raw UTF-8 save/load checks also remain required. All other payload, context, coder, profile, threshold, carrier and recovery rules are unchanged. Qualification is 40 exact sequence-text recoveries, 40 exact fixed-PNG recoveries, correctly accounted static/timing outcomes, and complete allocated trace/control/lossless evidence—not 152 successful recoveries. See `configs/v1_gpu_authorization.json` and the eventual `notes/v1_qualification_results.md` for execution and freeze evidence.
