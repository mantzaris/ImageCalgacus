...
def read_profile(path):
    ...
    if profile.get("protocol") != "imagecalgacus-v0-fixed16" or profile.get("packet_bytes") != 292:
...
def budget_state():
    path = ROOT / ".runtime/gpu_budget.jsonl"
...
def run_budgeted(command, label):
    directory = ROOT / ".runtime"
    directory.mkdir(exist_ok=True)
    with (directory / "gpu.lock").open("a") as lock:
        fcntl.flock(lock, fcntl.LOCK_EX | fcntl.LOCK_NB)
        used, records = budget_state()
        remaining = GPU_LIMIT_SECONDS - used
        if remaining <= 0:
            raise RuntimeError("two GPU-hour V0 allowance exhausted")
        ident = "%03d-%s" % (len(records) // 2 + 1, label)
        ...
        append({"event": "started", "id": ident, "utc": datetime.now(timezone.utc).isoformat(),
                "command": command, "remaining_seconds": remaining})
        env = os.environ.copy()
...
def main():
    ...
    if args.status:
        ...
        print(json.dumps({"completed_seconds": used, "active_elapsed_seconds": active_seconds,
    ...
    sys.exit(run_budgeted(command, args.label))
