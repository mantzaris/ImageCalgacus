...
Neural Linguistic Steganography \cite{ziegler} encodes a message through arithmetic decoding and recovers it by replaying arithmetic encoding. Our comparator follows that distribution aware approach with an explicit finite message adaptation.
...
Quantization index modulation selects reconstruction sets to balance embedding distortion and robustness \cite{qim}. Syndrome trellis codes minimize additive embedding costs \cite{stc}. HiDDeN and StegaStamp train encoders and decoders with distortion channels \cite{hidden,stegastamp}. Our photograph construction instead rebuilds pretrained model rank partitions within an invariant coarse context. Nearest-candidate selection bounds distortion, and delivered pixels suffice to reconstruct the context. This connects the Calgacus-style ranks to cover modification without a trained embedding network. The measured comparator is simple RGB parity, not these established systems.
...
For PNG carriers, PixelCNN++ \cite{pixelcnn} models a native $32\times32$ RGB canvas. A shared $1\times32$ RGB row occupies the first row. Only the remaining 31 rows are delivered, giving 992 pixels or 2,976 channel values. Each channel value in $\{0,\ldots,255\}$ is observable. Generation and replay use raster order, with R then G then B within each pixel. The receiver reads an 8bit RGB PNG without palette conversion, alpha compositing or color transformations and prepends the shared row internally.
...
The held out images are 20 Fashion-MNIST test samples, two per class \cite{fashion}, BOX resized from $28\times28$ to $16\times16$ grayscale. Twenty nonoverlapping Gutenberg spans from Austen's \emph{Pride and Prejudice} and Doyle's \emph{The Adventures of Sherlock Holmes} occupy the 32to64 and 65to128byte bands, ten each. Lengths are 58to128 bytes, mean 93.2. Frozen hashes, indices/offsets and exclusions prevent duplicate or development overlap without inspecting outcomes. Fashion MNIST's MIT notice and Gutenberg attribution are retained. Text copyright varies by jurisdiction.
...
\begin{table*}[t]
\caption{Original generated carrier specification. Dimensions are width by height. Both directions share a 292byte packet, but carrier symbols and useful rate units differ. One sealed packet is shared across methods within each payload/context group.}
...
\label{sec:observer}
The observer knows the exact model, profile, eligibility and the scoring rules and, initially, the correct context, but not the key or source. Its a frozen whole carrier scores are
\begin{align}
S(y;c)&=-N^{-1}\sum_t\log_2 q_t(y_t),\\
R(y;c)&=N^{-1}\sum_t\log_2\rank_t(y_t).
\end{align}
Higher always indicates stego. AUC is the probability that stego score exceeds an ordinary score, with a half weight for ties. No score orientation or threshold is selected using held out labels. Scorable failed carriers remain included.
...
The lowest ordered development payload in each of three byte bands gives 32, 96 and 128 byte messages under row1. Fixed rank has the three matched timing repetitions per payload with alternating reference/graph order. The 32 byte case also tests gated and A1 once, giving 11 comparisons and 22 recoveries, and not 11 independent payloads.
...
All PNG methods recover 20/20, giving mean exact goodput of 0.25054 bits/channel [0.24731, 0.25349], or 0.75161 bits/pixel. Equal rates follow partly from the fixed full canvas and do not imply equal coder capacity. Fixed uses 584 packet channels, gating adds a mean 100.5 skips, and A1's mean packet span is 597.85 channels, including 89.65 zero bit positions. The ordinary completion fills the remaining channels.
...
Mean 'row2 minus row1' score changes are 0.610 bits/channel for controls, 0.334 for fixed, 0.513 for gated and 0.629 for A1. These descriptive shifts narrow the paired fixed-minus-control gap from 0.508 to 0.231. Larger control increases are consistent with, but do not explain, the AUC change. Row1 comes from the generated image and row2 from random RGB values, so the context correctness is not isolated from row statistics.
...
The model arm averages 70.255 dB PSNR and 0.9999664 SSIM, with a maximum observed channel change of two. Parity's maximum is one. The model arm changes 1.781\% of pixels and 0.613\% of channels. The packet rate is $2336/65536=0.0356445$ bits/pixel, with 58to128 useful bytes. A fresh process encoding and decoding average 6.998 and 6.884 seconds. These costs concern fixed context tile evaluation, not a speedup comparison with autoregressive generation.
...
Development isolates saved text serialization. Twenty image payloads under two prompts give 40 packet-paired cases. Complete-prefix filtering recovers 40/40, versus 22/40 for singleton filtering (8/20 forest, 14/20 soup). All 18 drift failures remain unrepaired. This supports the consistency requirement alongside published stepwise verification. The independent sample comprises 20 payloads, not 40. With twenty generated PNG resaves also recover exactly. JPEG, cropping and paraphrasing remain untested.
...
Invariant cover pixels enable reproducible model-rank embedding and exact held-out photograph recovery. The simple parity attains the same recovery with slightly better fidelity and lower cost. With combined evaluation makes saved-artifact recovery, observer knowledge and execution cost explicit, separately measurable endpoints.

\space
\section*{\uppercase{AI Assistance Disclosure}}
\label{sec:ai}
OpenAI Codex assisted with implementation, analysis scripts, and figure preparation \cite{codex}. 
{\small
