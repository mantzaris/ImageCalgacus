...
\caption{Actual artifacts selected by manifest order. A transports a canonical $16\times16$ grayscale array through a verbatim, incomplete excerpt of its 616-token UTF-8 carrier, recovering all 256 pixels rather than an arbitrary image file. B shows complete 63-byte source/recovered texts and the actual $32\times31$ RGB PNG. Pixels use nearest-neighbor enlargement. Each fresh receiver uses carrier, profile, shared context and key. The PNG row is withheld and prepended internally. Source equality is evaluated separately. The figure's full-file note denotes local author material, not a reviewer-access link.}
\label{fig:examples}
\end{figure*}
...
The plaintext contains an eight-byte big-endian version/type/length/dimensions header and a 256-byte zero-padded payload slot. Text dimensions are zero and canonical images are $16\times16$. Header widths and permitted values are detailed in the companion. No MIME field or payload sidecar is added.
