...
Framing/encryption adds 36 bytes. Texts leave a mean 162.8 padding bytes, while image sources fill the slot. The companion reports complete packet rates, serialized expansion and overlapping termination diagnostics separately from useful goodput.
...
Descriptive score shifts narrow the paired mean fixed-minus-control gap from 0.508 to 0.231 bits/channel. Larger control increases are consistent with this movement but do not explain its cause. Row1 comes from a generated image and row2 from random RGB values, so correctness is not isolated from row statistics. The companion retains all four category means.
