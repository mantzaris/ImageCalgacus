...
The held-out images are 20 Fashion-MNIST test samples, two per class \cite{fashion}, BOX-resized from $28\times28$ to $16\times16$ grayscale. Twenty nonoverlapping Gutenberg spans from Austen's \emph{Pride and Prejudice} and Doyle's \emph{The Adventures of Sherlock Holmes} occupy two byte bands, ten each. Lengths are 58--128 bytes, mean 93.2. Frozen hashes, indices/offsets and exclusions prevent duplicate or development overlap without inspecting outcomes. Fashion-MNIST's MIT notice and Gutenberg attribution are retained; text copyright varies by jurisdiction.
