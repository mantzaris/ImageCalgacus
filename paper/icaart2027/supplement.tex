...
\abstract{This companion supplies a transport diagram, expanded numerical tables, development serialization comparisons, arithmetic diagnostics and reproducibility details for the main paper. It adds no experiment, model inference, hypothesis test or independent sample. Prospective held-out results, development qualification, repeated development GPU timings and repeated context scores are kept separate. The main paper contains the protocol, principal outcomes and central limitations and does not depend on this companion being reviewed. A separate supplementary upload for an ICAART regular paper has not been verified in the official instructions. This document is therefore prepared as a companion artifact, not represented as accepted supplementary submission material.}
\onecolumn\maketitle\normalsize\setcounter{footnote}{0}\vfill

\section{\uppercase{Transport and Independent Verification}}
The actual bidirectional examples appear in Figure 1 of the main paper. Figure~\ref{fig:method} summarizes their shared transport and verification contract. A source-image PNG displays the canonical payload, but equality is evaluated on all 256 recovered grayscale bytes. Text equality concerns the complete literal UTF-8 bytes. Visual similarity or authentication alone does not substitute for these comparisons. Drafting assistance throughout this companion is disclosed at its end \cite{codex}.

\begin{figure*}[t]
\centering
\input{figures/method_diagram}
\caption{Common transport and independent verification. The same packet contract feeds a text language model or a pixel autoregressive model. Only saved UTF-8 or PNG crosses the carrier boundary. A fresh receiver uses its declared model/profile, shared context and key. Source bytes and sender diagnostics remain outside its inputs. A separate evaluator checks source equality after receiver exit. The conditioning row is prepended internally, not delivered as an extra PNG row.}
\label{fig:method}
\end{figure*}

\begin{figure*}[t]
\centering\includegraphics[width=\textwidth]{figures/figure3_detectability.pdf}
...
\input{tables/supp_table2_v2_outcomes}
\end{table*}
...
\input{tables/supp_table3_detectability}
\end{table*}
...
\begin{figure*}[t]
\centering\includegraphics[width=\textwidth]{figures/figureS1_arithmetic_capacity.pdf}
...
\input{tables/supp_tableS1_overhead}
\end{table*}
...
\begin{figure*}[t]
\centering\includegraphics[width=\textwidth]{figures/figureS2_text_consistency.pdf}
...
\input{tables/context_comparison}
\end{table*}

\begin{figure*}[t]
\centering\includegraphics[width=\textwidth]{figures/score_shifts.pdf}
...
\input{sections/cover_supplement}
